...
/** 
 * Generic Doubly Node List class with one sentinel node in tail.
 */

public class DoublyNodeListSentinel<T> {
  private int nofElements;
  private DNode<T> head, tail;

  public DoublyNodeListSentinel() {
    /* at first head equals tail
     */
    nofElements = 0;
  ...
  public DNode<T> insertAfter(DNode<T> p, T element) {
    DNode<T> q;
    if(p==tail) {
      System.out.println("Unable to insert after sentinel node (tail)");
      return null;
    }
    else {
  ...
  public T delete(DNode<T> p) {
    ...
    if(p==tail) {
      System.out.println("Unable to delete sentinel node (tail)");
      return null;
    }
    if( p!=head ) {
      nofElements--;
   ...
   */
  public void catenate(DoublyNodeListSentinel<T> s) {
    if(s.isEmpty()) {
      return;
    }
